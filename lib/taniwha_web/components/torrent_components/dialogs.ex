defmodule TaniwhaWeb.TorrentComponents.Dialogs do
  ...
  def confirmation_dialog(%{confirm_action: {:erase_with_data, _hash, _base_path}} = assigns) do
    ~H"""
    <.dialog_shell warning_icon={true}>
      <:title>Remove torrent and delete files?</:title>
      <:body>
        <p>
  ...
  def confirmation_dialog(%{confirm_action: {:bulk_erase_with_data, _hashes, _paths}} = assigns) do
    ~H"""
    <.dialog_shell warning_icon={true}>
      <:title>Remove {length(elem(@confirm_action, 1))} torrents and delete their files?</:title>
      <:body>
        <p>
  ...
  def confirmation_dialog(assigns) do
    ~H"""
    <.dialog_shell warning_icon={false}>
      <:title>Remove torrent?</:title>
      <:body>
        <p :if={@torrent_name}>
  ...
  attr :warning_icon, :boolean, default: false
  slot :title, required: true
  slot :body, required: true
  slot :action_button, required: true

  defp dialog_shell(assigns) do
    ~H"""
    <div
        ...
        >
          <div class="px-5 pt-4 pb-[14px] border-b border-gray-200 dark:border-gray-700">
            <h2
              ...
              class={[
                "text-[14px] font-semibold text-gray-900 dark:text-gray-50",
                @warning_icon && "flex items-center gap-2"
              ]}
            >
              <svg
                :if={@warning_icon}
