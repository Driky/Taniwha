defmodule Taniwha.FileSystem do
  ...
  exist — `File.rm_rf/1` is idempotent). Returns one of the following errors:
    ...
    * `{:error, :path_outside_downloads_dir}` — resolved path is not inside
      `base_dir`, or is `base_dir` itself
  ...
  @spec safe_delete(String.t(), String.t()) ::
          :ok | {:error, :invalid_path | :path_outside_downloads_dir | File.posix()}
  ...
  def safe_delete(path, base_dir) do
    ...
    cond do
      resolved == expanded_base ->
        {:error, :path_outside_downloads_dir}
